...
export default function DraftModal({ pool, gameType, existingTeamA, onCancel, onConfirm }: DraftModalProps) {
  ...
  // Si ya hay un equipo A, los jugadores del pool son automáticamente para el equipo B
  const isTeamALocked = !!existingTeamA;
  const teamBPlayers = pool.filter(p => !teamAPlayers.find(tp => tp.id === p.id));

  const togglePlayer = (player: Player) => {
    if (isTeamALocked) return; // No se puede editar equipo A si ya existe
  ...
  const handleGenerateNames = async () => {
    ...
    try {
      // Si el equipo A ya existe, solo generamos para el B
      if (!isTeamALocked) {
  ...
  // Efecto inicial: si el equipo A está bloqueado, auto-generar nombre para B
  useEffect(() => {
  ...
  // Efecto cuando se seleccionan 5 jugadores para el equipo A
  useEffect(() => {
  ...
  const canConfirm = (isTeamALocked || teamAPlayers.length === 5) && teamAName && teamBName;
      ...
      <DialogContent className="max-w-4xl bg-background border-border p-0 overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row h-[500px]">
          ...
          <div className="w-full md:w-1/2 p-6 border-r border-border overflow-y-auto custom-scrollbar">
            <h3 className="text-sm font-bold text-muted-foreground uppercase mb-4 tracking-tighter">
              {isTeamALocked ? "Nuevos Retadores (Equipo B)" : `Pool de Retadores (${pool.length})`}
            </h3>
                ...
                return (
                  ...
                  >
                    ...
                    <Badge variant={isSelectedForA ? "default" : isForcedB ? "secondary" : "outline"} className="font-bold">
                      {isSelectedForA ? "EQUIPO A" : isForcedB ? "EQUIPO B" : "PENDIENTE"}
                    </Badge>
                  </button>
                );
              })}
            ...
            {isTeamALocked && (
              <p className="mt-4 text-xs text-muted-foreground italic">
                * Los retadores se asignan automáticamente al Equipo B porque el Equipo A permanece en cancha.
              </p>
            )}
          </div>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <h4 className="font-black uppercase tracking-widest text-primary flex items-center gap-2">
                  Equipo A {isTeamALocked && "(EN CANCHA)"}
                </h4>
                <Badge className="gold-gradient">{isTeamALocked ? 5 : teamAPlayers.length} / 5</Badge>
              </div>
              <div className="p-4 bg-background border border-border rounded-xl min-h-[100px]">
                ...
                <div className="flex flex-wrap gap-2">
                  {(isTeamALocked ? existingTeamA?.players : teamAPlayers)?.map(p => (
        ...
        <DialogFooter className="p-6 border-t border-border bg-card/50">
          ...
          <Button 
            ...
            onClick={() => onConfirm(isTeamALocked ? existingTeamA!.players : teamAPlayers, teamBPlayers, teamAName, teamBName)}
